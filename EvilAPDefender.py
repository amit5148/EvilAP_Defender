    ...
    try:
        ...
        if cursor.rowcount > 0:
            ...
            for row in ssids_data:
                ...
                conn.execute(cmd, (row[1],row[2],row[4],row[5],row[6],row[7]))
...
def cmd_iwconfig():
    mons = []
    ifaces = {}
    iwconf = Popen(['ifconfig'], stdout=PIPE)
...
def insert_ap(pkt):
    try:
        ...
        #conn.execute("select * from whitelist where ssid = '" + ssid + "'")
        #if cursor.rowcount > 0:
        for item in OUIs:
            cmd = "insert into ssids_OUIs (mac, ssid, oui) values(%s,%s,%s)"
            conn.execute(cmd, (bssid, ssid, item))
    ...
    try:
        ...
        try:
            ...
            for row in reader:   # iterates the rows of the file in orders
                ...
                #print ESSID
                #conn.execute('insert into ssids (mac,ssid,channel) values(ESSID,SSID,CHANNEL)')
                cmd = "insert into ssids (mac,ssid,pwr,channel,CIPHER,Enc,Auth) values(%s,%s,%s,%s,%s,%s,%s)"
                conn.execute(cmd, (row[0].strip(), row[13].strip(), row[8].strip(), row[3].strip(), row[6].strip(), row[5].strip(), row[7].strip()))
                #conn.execute('insert into ssids (mac,ssid,pwr,channel,CIPHER,Enc,Auth) values(' + "'" + row[0].strip() + "'" + ',' + "'" + row[13].strip() + "'" + ',' + row[8].strip() \
                #+ ',' + row[3].strip() + ',' + "'" + row[6].strip() + "'" + ',' + "'" + row[5].strip() + "'" + ',' + "'" + row[7].strip() + "'" + ')')
    ...
    try:
        while True:
            ...
            if choice == "1":
                ...
                #conn.execute("select * from ssids where mac='{}')
                cmd = "select * from ssids where ssid=%s"
                conn.execute(cmd, (SSID,))
                if cursor.rowcount > 0:
                ...
                while True:
                    ...
                    if option == "1":
                        ...
                        conn.execute(cmd, (deauth_repeat,))
                        conn.commit()
                    elif option == "2":
                        ...
                        conn.execute(cmd, (admin_smtp_username,))
                        ...
                        conn.execute(cmd, (admin_smtp_password,))
                        conn.commit()
                    else:
                        break
